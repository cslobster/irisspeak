...
**Stale premise, flagged 2026-07-29:** this design was written assuming `corpus.ts` still has the exact→word-boundary→MiniLM-cosine retriever to reuse. That retriever (and its `@xenova/transformers` dependency) was deliberately deleted in `6dc8371` ("Un-bypass AAC card generation") the same day, in favor of handing the LLM the *entire* category vocab directly and letting it pick from a ranked list — which already avoids the old "correct after generation" relevance problem at the corpus's current size (~721 topic+action words). See `CONTEXT.md`'s Corpus Enrichment entry for the full story. Below is the original design, kept for reference — implementing it now means writing new narrowing logic (semantic or keyword-based), not reusing removed code, and is probably only worth doing once Tier 1 grows the corpus large enough that the full-vocab-in-prompt approach stops being cheap.
