...
CONTR = {"i'm":"i am","i've":"i have","i'll":"i will","i'd":"i would","don't":"do not","can't":"can not",
         ...
         # added 2026-09-10 after the full missing-token audit (vocab/corpus_missing_full.json)
         "how's":"how is","you'll":"you will","you've":"you have","you'd":"you would","he'll":"he will","she'll":"she will",
         "it'll":"it will","we'll":"we will","they'll":"they will","that'll":"that will","here's":"here is","who's":"who is",
         "there'll":"there will","we've":"we have","they've":"they have","we'd":"we would","he'd":"he would","she'd":"she would",
         "they'd":"they would","it'd":"it would","y'all":"you all","ain't":"is not","hadn't":"had not","mustn't":"must not"}
...
def normalise(text, known=frozenset()):
    ...
    toks = []
    for w in t.split():
        # possessives: "emily's" -> <name>, "dad's" / "other's" -> the base card (the 's carries no card of its own)
        base = w[:-2] if w.endswith("'s") and len(w) > 3 and w not in CONTR else None
        if w in names or (base and base in names): toks.append("<name>"); continue
        if base and base in known: toks.append(base); continue
        nw = num_to_words(w)
