...
### Vocabulary Coverage
Whether *any* card exists to answer a given question's theme, independent of which child is asking — a content-completeness property of the shared corpus/folder catalog, not a per-child personalization concern. Partially solved today: 5 of the ~45 folders in `web-client/public/cboard_folders.json` (790 total folder+word rows) are curated into `data/folder_cards.yml` with a hybrid LLM-choice + regex-keyword backstop (see Folder Card) — the other ~40 folders exist in the catalog but have no curated trigger. The universal last-resort fallback is the "View all words" search overlay, reachable from every child turn unconditionally (not gated behind any folder being detected that turn, and doesn't consume topic-tile budget) — this already guarantees a child can always find *something*, even though it's slow and defeats the point of a "smart" AAC as the primary path.
