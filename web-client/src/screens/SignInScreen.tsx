...
export function SignInScreen() {
  ...
  // Two ways in: Google (primary) and the username + login code an admin issued.
        ...
        <div className="bg-white rounded-3xl p-7 w-full max-w-sm" style={{ boxSizing: 'border-box', border: '3px solid #000', borderBottomWidth: 8 }}>
          ...
          {isAuthorizing ? (
            ...
            <div className="flex flex-col gap-4">
              {/* Google is the primary way in; the username + login code form is for accounts an admin created. */}
              <GoogleButton href={googleSignInUrl()} />

              {!showPassword ? (
                <button onClick={() => setShowPassword(true)} className="text-sm font-semibold text-slate-500 hover:text-slate-700 transition py-1">
