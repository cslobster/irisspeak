...
export const PANEL = { topic: 12, action: 3, emotion: 3 };          // phones: Topic 4 x 3, Action 1 x 3, Feeling 1 x 3
export const PANEL_BIG = { topic: 15, action: 3, emotion: 3 };      // iPad landscape and desktops: a fifth Topic column
...
class LocalApi {
  ...
  private panel = { ...PANEL_BIG };   // the desktop board; CompactSession (phones) slices its own 12 / 3 / 3
  ...
  async inferSentence(_id: string, again = false): Promise<{ sentence: string; source: 'cloud' | 'device' }> {
    ...
    try {
      const setting = getProfile().setting;
      sentence = await realiser.realise(labels, cur.question, again ? { avoid: cur.candidates, sample: true, setting } : { setting });
