...
// camp's own intake; medical/diagnosis data is excluded from the app entirely). Submissions
// land in a 'pending' status (see CONTEXT.md's Custom Vocabulary Word / signup wizard design)
// until an admin approves them — this screen never issues a working login on its own.
      ...
      <div className="relative min-h-screen overflow-hidden flex items-center justify-center px-6" style={{ background: '#f0ebe1' }}>
        <div className="bg-white rounded-3xl p-8 w-full max-w-sm text-center" style={{ boxSizing: 'border-box', border: '3px solid #000', borderBottomWidth: 8 }}>
          ...
          <p className="text-sm text-slate-600 mb-4">
            Your signup is waiting for approval. Once it's approved, sign in with:
          </p>
  ...
  return (
    <div className="relative min-h-screen overflow-hidden py-10 px-6" style={{ background: '#f0ebe1' }}>
      <div className="max-w-lg mx-auto">
        ...
        <p className="text-center text-slate-600 font-semibold mb-6 text-sm">
          Tell us about your child — accounts need approval before they're active.
        </p>

        <div className="bg-white rounded-3xl p-7" style={{ boxSizing: 'border-box', border: '3px solid #000', borderBottomWidth: 8 }}>
          {error && (
            <p className="mb-4 text-sm font-semibold text-[#f09281] bg-[#f09281]/10 rounded-xl px-3 py-2">{error}</p>
          )}
          <div className="flex flex-col gap-4">
            <Field label="Child's name">
              <input className={inputClass} value={childName} onChange={(e) => setChildName(e.target.value)} autoFocus />
            </Field>
            <div className="flex gap-3">
              ...
              <Field label="Gender">
                <select className={inputClass} value={childGender} onChange={(e) => setChildGender(e.target.value as ChildGender)}>
            ...
            <Field label="Interests (comma separated)">
              <input
                className={inputClass}
                placeholder="e.g. dinosaurs, Bluey, Legos"
                value={interests}
            ...
            <Field label="Anything else we should know?">
              <textarea
                className={inputClass}
                rows={4}
                placeholder="Sensory preferences, what helps them stay calm…"
                value={notes}
            ...
            <button
              onClick={submit}
              disabled={!childName.trim() || !loginCode.trim() || submitting}
        ...
        <Link to="/" className="block text-center text-sm font-semibold text-slate-500 mt-5 hover:text-slate-700 transition">
