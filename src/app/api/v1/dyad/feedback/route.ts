 ...
 *          choice: 'dislike' | 'own_answer', disliked?: string[] (card ids crossed out), answer?, model_version?, client?, timestamp? }
...
export async function POST(req: Request) {
  ...
  if (choice !== 'dislike' && choice !== 'no_cards' && choice !== 'own_answer') return badRequest("choice must be 'dislike' or 'own_answer'");
  ...
  const disliked = Array.isArray(body.disliked) ? body.disliked.map(String).slice(0, 60) : [];
  const id = nanoid();
  try {
    await sql`INSERT INTO board_feedback (id, dyad_id, session_id, setting, question, candidates, prefix, disliked, choice, answer, model_version, client, timestamp)
              VALUES (${id}, ${dyad.id}, ${body.session_id ? String(body.session_id) : null}, ${setting}, ${question},
                      ${JSON.stringify(candidates)}::jsonb, ${JSON.stringify(prefix)}::jsonb, ${JSON.stringify(disliked)}::jsonb, ${choice}, ${answer},
...
export async function GET(req: Request) {
  ...
  const dyad = await dyadFromRequest(req);
  if (!dyad) return unauthorized();
  const rows = await sql`SELECT id, session_id, setting, question, candidates, prefix, disliked, choice, answer, model_version, timestamp
