...
export async function GET(req: Request, ctx: { params: { sessionId: string } }) {
  ...
  const rows = (await sql`
    SELECT id, dyad_id, topic_category, subtopic, subtopic_description, local_timezone,
           status, started_timestamp, ended_timestamp, num_turns, rating, title, client
    FROM session WHERE id = ${ctx.params.sessionId} AND dyad_id = ${dyad.id} LIMIT 1
  `) as any[];
  const r = rows[0];
  if (!r) return notFound();
  return ok({
    id: r.id, dyad_id: r.dyad_id,
    topic: { category: r.topic_category, subtopic: r.subtopic, subtopic_description: r.subtopic_description },
    status: r.status, local_timezone: r.local_timezone,
    started_timestamp: Number(r.started_timestamp || 0),
    ended_timestamp: r.ended_timestamp ? Number(r.ended_timestamp) : null,
    num_turns: r.num_turns, rating: r.rating ?? null, title: r.title ?? null, client: r.client ?? null,
  });
}
