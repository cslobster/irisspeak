...
import { endSession } from '@/lib/moderator';
...
export async function PUT(req: Request, ctx: { params: { sessionId: string } }) {
  ...
  try { await endSession(ctx.params.sessionId, dyad); return noContent(); }
  catch (e: any) { return serverError(e?.message || 'end failed'); }
}
